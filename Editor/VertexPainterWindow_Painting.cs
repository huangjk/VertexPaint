...
namespace JBooth.VertexPainterPro
{
    public partial class VertexPainterWindow : EditorWindow
    {
        ...
        static void ColorRGBABlend(PaintJob j, int idx, ref object v, float r)
        {
            var s = j.stream;
            s.colors[idx] = Color.Lerp(s.colors[idx], s.colors[idx] + (Color)v, r);
        }
        ...
        public Lerper GetLerper()
        {
            ...
            switch (brushMode)
            {
                ...
                case BrushTarget.FullColor:
                    {
                        switch (brushColorMode)
                        {
                            case BrushColorMode.Normal:
                                return ColorRGBA;
                            case BrushColorMode.Blend:
                                return ColorRGBABlend;
        ...
        public BrushColorMode brushColorMode = BrushColorMode.Blend;
...
EditorUtility.SetSelectedWireframeHidden(r, hidden);
        ...
        void OnSceneGUI(SceneView sceneView)
        {
            ...
            else if (brushMode == BrushTarget.Color || brushMode == BrushTarget.FullColor || brushMode == BrushTarget.UV0_AsColor || brushMode == BrushTarget.UV1_AsColor
